...
export async function loadValidatedSifenPayload(
  ...
  const [itemsRes, clienteRes, configRes, electronicaRes] = await Promise.all([
    ...
    supabase
      .from("clientes")
      .select(
        "id, empresa, nombre_factura, nombre_contacto, nombre, ruc, documento, direccion, telefono, email, pais, sifen_receptor_extranjero, sifen_codigo_pais, sifen_tipo_doc_receptor, sifen_receptor_manual, sifen_receptor_naturaleza, sifen_ti_ope, sifen_num_id_de, sifen_direccion_de, sifen_num_casa_de, sifen_descripcion_tipo_doc"
      )
      .eq("id", clienteId)
      .eq("empresa_id", empresaId)
      .maybeSingle(),
