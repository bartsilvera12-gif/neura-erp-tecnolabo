  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-1 border-b border-slate-200">
        <Link href="/compras" className="border-b-2 border-transparent px-4 py-2 text-sm font-medium text-slate-500 transition-colors hover:text-[#17191C]">
          Compras
        </Link>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm ring-1 ring-[#1E2125]/15 sm:p-5 lg:p-6">
        ...
        <div className="mb-5 flex flex-wrap items-center gap-3 border-b border-slate-100 pb-5">
          ...
          <select value={filtroEstado} onChange={(e) => setFiltroEstado(e.target.value as EstadoOrdenCompra | "")}
            className="w-44 rounded-lg border border-slate-200 px-3 py-2 text-sm bg-white outline-none focus:ring-2 focus:ring-[#1E2125]/30">
              ...
              <thead className="border-b-2 border-[#1E2125]/40 bg-[#EEF0F2]">
                <tr>
                  {["N° OC", "Fecha", "Proveedor", "Ítems", "Total", "Estado", ""].map((h, i) => (
                    <th key={h} className={`px-3 py-3 text-xs font-bold uppercase tracking-wide text-[#17191C] ${i === 3 || i === 4 ? "text-right" : i === 5 || i === 6 ? "text-center" : "text-left"}`}>{h}</th>
                  ))}
                </tr>
              </thead>
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <div className="inline-flex items-center gap-3">
                        <Link href={`/compras/ordenes/${encodeURIComponent(g.numero_oc)}`}
                          className="text-xs font-semibold text-[#17191C] hover:underline">Ver</Link>
